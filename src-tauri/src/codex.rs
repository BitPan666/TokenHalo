...
fn same_window(left: &UsageWindow, right: &UsageWindow) -> bool {
    left.window_seconds == right.window_seconds && left.resets_at == right.resets_at
}

fn push_unique(windows: &mut Vec<UsageWindow>, candidate: &Value) {
    let Some(window) = parse_window(Some(candidate)) else {
        return;
    };
...
fn collect_windows(rate_limit: &Value) -> Vec<UsageWindow> {
...
fn select_display_windows(windows: Vec<UsageWindow>) -> (Option<UsageWindow>, Option<UsageWindow>) {
    let five_hour = windows
        .iter()
        .position(|item| matches_duration(item, FIVE_HOURS_SECONDS));
    let weekly = windows
        .iter()
        .position(|item| matches_duration(item, ONE_WEEK_SECONDS));
    let primary_index = five_hour
        .or(weekly)
...
fn safe_http_failure(status: reqwest::StatusCode) -> (&'static str, &'static str) {
    match status.as_u16() {
        ...
        429 => (
            "unavailable",
            "Codex usage service is rate limited. It will retry automatically.",
        ),
        _ => (
            "unavailable",
            "Codex usage service is temporarily unavailable.",
        ),
    }
}
...
pub async fn fetch_snapshot(client: &reqwest::Client) -> ProviderSnapshot {
    ...
    let usage: Value = match limited_json(usage_response).await {
        ...
        Err(_) => {
            return ProviderSnapshot::failure(
                "unavailable",
                "Codex usage response format has changed.",
            )
        }
    };
    let rate_limit = usage
        ...
        .unwrap_or(&usage);
    let (short_window, weekly_window) = select_display_windows(collect_windows(rate_limit));
...
mod tests {
    ...
    #[test]
    fn selects_five_hour_primary_without_a_secondary() {
        let rate_limit = serde_json::json!({
            "primary_window": {
                "remainingPercent": 73,
                "windowSeconds": 18000,
                "resetsAt": "2026-07-25T13:31:12Z"
            }
        });
        let (primary, secondary) = select_display_windows(collect_windows(&rate_limit));
    ...
    #[test]
    fn selects_no_windows_when_the_response_has_no_usable_window() {
        let rate_limit = serde_json::json!({
            "primary_window": {
                "windowSeconds": 18000,
                "resetsAt": "2026-07-25T13:31:12Z"
            ...
            "windows": [
                null,
                {"remainingPercent": "not-a-number", "windowSeconds": 604800}
            ]
        });
        let (primary, secondary) = select_display_windows(collect_windows(&rate_limit));
