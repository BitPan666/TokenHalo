...
TokenHalo 是一个 Tauri 2 桌面悬浮窗，用本机 Codex Desktop 登录态只读查询 Codex 额度，并按当前登录套餐实际返回的窗口展示五小时或每周剩余额度、重置时间、重置机会和会员类型。
...
- 悬浮额度卡片：按 Codex usage service 实际返回的窗口展示五小时或每周剩余额度、重置时间和重置机会，不为缺失的周期填充虚假数值。
