...
将当前基于 TokenHalo 开发的桌面应用统一重命名为 **TokenHalo**，副标题为 **Codex Usage Monitor**。改名覆盖用户可见品牌、代码包标识、构建产物、发布文档和后续开发约定，同时保留现有功能、偏好设置、Token 统计索引和必要的上游版权声明。
...
现有图标和截图如果不含 TokenHalo 字样则继续使用；本次不包含视觉品牌重设计。
...
- `tokenhalo` → `tokenhalo`；
- `tokenhalo_lib` → `tokenhalo_lib`；
- `TokenHaloPanel` → `TokenHaloPanel`；
- `TokenHalo/<version>` → `TokenHalo/<version>`；
- `app.tokenhalo.desktop` → `app.tokenhalo.desktop`。
...
1. MIT `LICENSE` 中原有的 `TokenHalo contributors` 版权声明必须保留，并新增 TokenHalo 贡献者声明。
2. 兼容迁移代码和迁移测试中必须保留旧 Bundle ID `app.tokenhalo.desktop`，仅用于定位旧配置目录。
3. Git 提交历史不重写，历史提交信息中的旧名称保持原样。
4. 若需说明项目来源，可在致谢或版权语境中使用旧名称，但不得将其作为当前产品名展示。

除上述允许项外，受版本控制的当前文件中不应残留 `TokenHalo`、`tokenhalo`、`tokenhalo`、`TokenHalo` 或 `tokenhalo`。
...
- 旧目录：`${configDir}/app.tokenhalo.desktop`
...
2. 经用户明确确认后，将 GitHub 仓库从 `tokenhalo` 重命名为 `tokenhalo`，随后更新本地 `origin` URL 并验证 Issues、Releases 和 Actions 链接。
