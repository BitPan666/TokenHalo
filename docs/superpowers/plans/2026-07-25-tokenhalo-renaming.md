...
**Goal:** Rename the current application to TokenHalo across runtime identity, package metadata, documentation, artifacts, and repository links while preserving supported Quota Float preferences and Token statistics cache data.
...
- Preserve the original MIT `Quota Float contributors` copyright line and add a TokenHalo contributors line.
...
```rust
pub(crate) const LEGACY_IDENTIFIER: &str = "app.quotafloat.desktop";
...
Expected: FAIL on `Quota Float` / `app.quotafloat.desktop`.
...
```text
package.json name                    quota-float -> tokenhalo
package-lock.json root names         quota-float -> tokenhalo
Cargo.toml package name              quota-float -> tokenhalo
Cargo.toml lib name                  quota_float_lib -> tokenhalo_lib
Cargo.lock root package name         quota-float -> tokenhalo
tauri productName/title              Quota Float -> TokenHalo
tauri identifier                     app.quotafloat.desktop -> app.tokenhalo.desktop
index.html title                     Quota Float -> TokenHalo
DesignPlayground brand label         QUOTA FLOAT -> TOKENHALO
```
...
Rename both `QuotaFloatPanel` occurrences to `TokenHaloPanel`.
...
```bash
rg -l --hidden -i 'quota[-_ ]?float|QuotaFloat|quotafloat' \
  --glob '!node_modules/**' \
  --glob '!dist/**' \
...
quota-float-windows-unsigned.zip     -> tokenhalo-windows-unsigned.zip
quota-float-macos-universal-unsigned.zip -> tokenhalo-macos-universal-unsigned.zip
github.com/change-42-yhmm/quota-float -> github.com/change-42-yhmm/tokenhalo
Quota Float.app                      -> TokenHalo.app
quota-float.exe                      -> tokenhalo.exe
```
...
```bash
git grep -n -I -i -E 'Quota Float|quota-float|quota_float|QuotaFloat|quotafloat'
```
...
- the `Upgrading from Quota Float` compatibility sections in `README.md`, `docs/RELEASE.md`, and `docs/RELEASE_TEMPLATE.md`;
...
Expected: current-platform TokenHalo bundle/executable paths; no newly built Quota Float bundle.
...
```text
current repository: change-42-yhmm/quota-float
...
```bash
gh repo rename tokenhalo --repo change-42-yhmm/quota-float
```
...
- that users should verify TokenHalo before uninstalling Quota Float.
